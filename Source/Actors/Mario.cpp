        ...
        , mJumpSpeed(jumpSpeed)
{
    ...
    mDrawComponent = new DrawAnimatedComponent(this, "../Assets/Sprites/HeroPack/texture.png", "../Assets/Sprites/HeroPack/texture.json");
...
void Mario::OnUpdate(float deltaTime)
{
    ...
    mPosition.x = Math::Max(mPosition.x, GetGame()->GetCameraPos().x);
    ...
    if (mPosition.y > GetGame()->GetWindowHeight()) {
        Kill();
    }
...
void Mario::ManageAnimations()
{
    ...
    if (!mIsDead && mIsRunning && mIsOnGround) {
        mDrawComponent->SetAnimation("run");
    }
    if (!mIsDead && mIsOnGround && !mIsRunning) {
        mDrawComponent->SetAnimation("idle");
    }
    if (!mIsDead &&! mIsOnGround) {
...
void Mario::OnVerticalCollision(const float minOverlap, AABBColliderComponent* other)
{
    ...
    if (other->GetLayer() == ColliderLayer::Enemy) {
        other->GetOwner()->Kill();
        mRigidBodyComponent->SetVelocity(Vector2(0,mJumpSpeed / 2));
    }else {
        other->GetOwner()->OnVerticalCollision(minOverlap, this->GetComponent<AABBColliderComponent>());
    }
}
